...
The goal of HBMAP is to implement a model-based clustering approach through hierarchical Bayesian mixtures for high-throughput mapping of axon projections. The model allows for identifying groups of neurons with unique projection patterns across different brain regions, and investigates differences across experiments.
...
You can install the development version of HBMAP from [GitHub](https://github.com/) with:
...
# Warning on Spike-In Normalization

Spike-in normalization is a standard step in the analysis of MAPseq data, but 
can introduce extremely low / fractional barcode counts which conflict with
core HBMAP functionality. 

We currently recommend scaling your MAPseq data post spike-in normalization by
some constant (e.g. 1000, or the spike-in value of the negative control) prior 
to HBMAP analysis, as shown in the example below:

```{r}

# apply spike_in normalization to MAPseq data.frame
norm_data = spike_norm(raw_data, spike_in_counts)

# extract negative control spike_in count
negative_control_spike_value = spike_in_counts$negative_control

# scale data by negative control spike_in_count
scaled_data = norm_data * negative_control_spike_value
