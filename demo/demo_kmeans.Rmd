...
Implement the k-means model with varying choices of data transformation: row-sum normalization, log of row-sum normalization, max-row normalization, cosine normalization.

```{r kmeans}
set.seed(43)
Z <- k_means_exon(Y = data_Hans, k = 20, trans = 'log row-sum', restart = 20, iter.max = 100)
...
rm(Z_updated, C_cumsum, unique_cl, Z_unlist, Z_updated, j, J)
```
